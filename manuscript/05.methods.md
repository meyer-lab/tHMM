...
We use multivariate distributions to represent physical characteristics of the cells within the lineages, also known as emissions. The synethic data we used for evaluating the model were created for two scenarios. Considering 1- cell fate and intermitotic times as only observations, 2- phase specific fate and time durations when a cell cycle is assumed to be composed of two general phases (G1 and S/G2). We used a common emissions matrix, E [[0.99, 8, 6], [0.75, 8, 1]], for Figures @fig:prop4, @fig:real_5, @fig:performUncenSingle, @fig:performUncenMulti, and @fig:performCenMulti. The parameters represent the cell fate, cell lifetime, and scale for the cell lifetime respectively, with the number of emissions reflecting the number of states.  Similarly, we used another common emissions matrix, E2 [[0.99, 0.95, 8, 7, 4, 2], [0.95, 0.9, 6, 4, 3, 5]], for Figures @fig:censor, @fig:performSyn, @fig:uncenSingle, @fig:uncenMulti, and @fig:cenMulti. The parameters represent cell fate in G1 and G2 phase, cell lifetime in G1 and G2 phase, and two scales for the lifetime in G1 and G2, respectively. However, Figures @fig:wass, @fig:wass1, and @fig:wass2 use a unique set of emissions, in which the values for the gamma shape parameter are varied in order to increase the Wassertein distance between the two cell states, allowing us to measure state assignment accuracy. Likewise, Figure @fig:sAIC also uses a unique set of values for the emissions matrix in order to simulate varying states for the AIC calculation. 

Based on the emissions observations, we have censored the data for certain figures so that the daughter cells of a dead cell and cells that divide past the experiment time are specifically used for calculations with a censored estimator. 
