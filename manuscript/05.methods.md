...
From distribution analysis of the phase lengths, we realized the shape parameter of the gamma distribution remains fairly constant over different conditions while the scale paramter changes. This way, instead of $ k \times (k - 1) + (k - 1) + k \times (m) $ degrees of freedom, we will have $4 \times k$ where $k$ is the number of states and $m$ is the number of parameters associated with observation distribution. -->
