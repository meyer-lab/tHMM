...
In order to avoid underflow problem to calculate these two probability matrices, Durand et al \cite{durand} proposed \textit{upward-downward} algorithm for smoothed probabilities in tree hidden Markov model. To explain that we would need to indtroduce the following expressions:
...
$$ D_{KL}(P || Q) = \sum P(x) log\Big(\frac{P(x)}{Q(x)}\Big)$$