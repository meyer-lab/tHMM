...
#### Challenges in experimental data structure  
In single-cell experimental data collection, there are obsticles to obtain a full lineage. The following are three general reasons:   
1. Selection bias  
...
references: [@doi:10.1016/j.spa.2015.11.009]  
...
Correction regarding small data:  
TBW

__Wasserstein distance:__  
Wasserstein or Kantorovich–Rubinstein metric, is a measure of distance between two distributions. Let $P$ and $Q$ be two distributions with finite second moments. The Wasserstein distance between these two distributions is defined as:  
<center>$$ W^2_2(P, Q) = inf\{E(\zeta - \eta)^2\} = \int_0^1 |P^{-1} (t) - Q_{-1} (t) |^2 dt$$<center>  
where the infimum is taken over all pairs of random variables $$(\zeta, \eta)$$ with distributions $$(P, Q)$$, respectively.   

Here we want to show if the two states of the cells in the data, are far from each other, meaning, the cells in the two states are more different, then the model is better at distinguishing the two states, hence the accuracy of parameter estimation and state assignment is higher.  
