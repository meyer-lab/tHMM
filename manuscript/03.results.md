...
### Identifying heterogeneity in cell populations; importance of the model

[//]: # (Figure 1)

As a motivating example, we present two cases of cell population growth over time, one where the population is homogeneous and all cells show consistent behaviour across their observed phenotypes that we consider cell lifetime as an example (Figure1a), and one where the population can be clustered into at least two subpopulations, each with different growth rates (Figure 1b). Over time, the two populations look similar, with regards to thier size, if they are both assumed to grow exponenially. In the two cases, the initial and end-point cell numbers are almost equal, hence, the growth rate for both cases would be equal. Utilizing the tHMM model we have built, we are not only able to capture the heterogeneity present in the second population, but also quantify the relative presence of these two states, and summarize the characteristics of the observed phenotypes in those states. While the growth rate (or doubling time) for both models for the homogeneous case is alike, the exponential growth model fails in calculating the growth rate in the second case correctly, where our model can successfully disentangle the heterogeneous population into two states and report their growth rates, separately.
