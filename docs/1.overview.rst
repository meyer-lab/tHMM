...
.. code:: ipython3
    ...
    tHMMobj_list, pred_states_by_lineage_by_conc, LL = Analyze_list(Control, desired_num_states=3)
    # in this example, we are running the model with 3 states.

    print("the likelihood of having 3 states: ", LL)
    print("BIC value for each lineage tree: ", [tHMMobj_list[i].get_BIC()])
To find out the likelihood of running the model with different number of states, in parallel (to minimize the time of running) 
we can use ``run_Analyze_over()``. 