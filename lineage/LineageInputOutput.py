...
def import_exp_data(path):
    ...
    while lPos < len(data):
        ...
        if lPos < len(data) and math.isfinite(data[lPos][1]):
            ...
            if (
                exp_time == -1
                and not math.isnan(data[lPos][1 + 2])
                and parentCell.left is None
                and parentCell.right is None
            ):
            ...
            # [x  y]/[x y  ] case (general)
                ...
                if data[lPos][1] == data[lPos][2]:
                    ...
                    if math.isnan(data[lPos][1 + 1]):
                        parentCell.obs[1] = (
                            float("nan") if (data[lPos][1 + 2] == exp_time) else 1
                        )  # survived/did not observe G2
                        parentCell.obs[3] = (
                            data[lPos][1 + 2]
                            if (math.isnan(parentCell.obs[2]))
                            else data[lPos][1 + 2] - parentCell.obs[2]
                        )  # Time spent in G2
                    ...
                    # [x y  ] case
                        ...
                        parentCell.obs[3] = (
                            data[lPos][1 + 1]
                            if (math.isnan(parentCell.obs[2]))
                            else data[lPos][1 + 1] - parentCell.obs[2]
                        )  # Time spent in G2
...
def tryRecursion(
    ...
    # find lower daughter
    daughterCell.right = tryRecursion(
        pColumn,
        lower,
        parentPos,
        daughterCell,
    ...
    if (
        exp_time == -1
        and not math.isnan(data[parentPos][pColumn + 2])
        and daughterCell.left is None
        and daughterCell.right is None
    ):
