...
def figure_maker(ax):
    ...
    x1_gen = [i + 1 for i in list(range(len(hist_gen_uncensored[1])))]

    i = 0
    ax[i].set_xlim([-0.01, 13])
    ax[i].xaxis.set_major_locator(MaxNLocator(integer=True))
    ax[i].set_xlabel(r'Generation')
    ax[i].bar(x0_gen, hist_gen_uncensored[0], color='#F9Cb9C', label='Resistant')
    ax[i].bar(x1_gen, hist_gen_uncensored[1], bottom=hist_gen_uncensored[0], color='#A4C2F4', label='Susceptible')
    ax[i].set_ylabel('Number of alive cells')
    ax[i].set_title('Uncensored population growth')
    ax[i].grid(linestyle='--')

    y_0_gen_p = [a / (a + b) for a, b in zip(hist_gen_uncensored[0], hist_gen_uncensored[1])]
    y_1_gen_p = [b / (a + b) for a, b in zip(hist_gen_uncensored[0], hist_gen_uncensored[1])]
