...
def accuracy():
    ...
    def genF(x): return LineageTree.init_from_parameters(pi, x, E2, max_desired_num_cells)
    def genC(x): return LineageTree.init_from_parameters(pi, x, E2, max_desired_num_cells, censor_condition=3, desired_experiment_time=700)

    # Generate populations
    list_of_uncen_populations = [[genF(T) for _ in range(10)] for T in list_of_Ts]
    list_of_populations = [[genC(T) for _ in range(10)] for T in list_of_Ts]

    percentageS1un, _, output_un, _ = commonAnalyze(list_of_uncen_populations, 2, xtype="prop", list_of_fpi=list_of_fpi, list_of_Ts=list_of_Ts)
    percentageS1, _, output_cen, _ = commonAnalyze(list_of_populations, 2, xtype="prop", list_of_fpi=list_of_fpi, list_of_Ts=list_of_Ts)
...
def figureMaker7(ax, un_accuracy_df, accuracy_df):
    ...
    ax[i].set_ylim(bottom=0.0, top=101.0)
