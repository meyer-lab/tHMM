...
for lins in gemc_tHMMobj_list[1].X:
    if lins.output_lineage[0].state == 0:
        ...
        st3.append(lins)
gemc_tHMMobj_list[1].X = st1[0:min(50, len(st1))] + st2[0:min(50, len(st2))] + st3[0:min(50, len(st3))]
...
def makeFigure():
    ...
    for i, X in enumerate(gemc_tHMMobj_list[1].X):
        ax[i].axis('off')
        plotLineage(X, ax[i])
    return f