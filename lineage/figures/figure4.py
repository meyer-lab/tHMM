...
def figureMaker3(ax, x_Sim, output_Sim, x_Cen, output_Cen, xlabel="Number of Cells"):
    ...
    accuracy_sim_df = pd.DataFrame(columns=["Cell number", "State Assignment Performance"])
    accuracy_sim_df["Cell number"] = x_Sim
    accuracy_sim_df["State Assignment Performance"] = Accuracy_Sim

    accuracy_cen_df = pd.DataFrame(columns=["Cell number", "State Assignment Performance"])
    accuracy_cen_df["Cell number"] = x_Cen
    accuracy_cen_df["State Assignment Performance"] = Accuracy_Cen
    ...
    sns.regplot(x="Cell number", y="State Assignment Performance", data=accuracy_sim_df, ax=ax[i], lowess=True, marker='+', scatter_kws=scatter_state_1_kws)
    ...
    i += 1
    ax[i].axhline(y=100, ls='--', c='k', alpha=0.5)
    sns.regplot(x="Cell number", y="State Assignment Performance", data=accuracy_cen_df, ax=ax[i], lowess=True, marker='+', scatter_kws=scatter_state_1_kws)
    ax[i].set_xlabel(xlabel)
    ax[i].set_ylim(bottom=0, top=101)
    ax[i].set_ylabel(r"State Accuracy [%]")
