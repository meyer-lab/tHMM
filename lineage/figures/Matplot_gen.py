...
def Matplot_gen(ax, x, acc_h1, bern_MAS_h1, bern_2_h1, MASlocBern, locBern2, MASbeta, beta2, betaExp_MAS_h1, betaExp_2_h1, xlabel, FOM='E'):
    ...
    ax[0].set_ylim(0, 110)
    ax[0].errorbar(x, acc_h1, fmt='o', c='k', marker=".", label='Accuracy', alpha=0.5)
    ax[0].axhline(y=100, linestyle=(0, (3, 5, 1, 5, 1, 5)), linewidth=1, color='k')  # linestyle is dashdotdotted
    ...
    ax[1].errorbar(x, bern_MAS_h1, fmt='o', c='b', marker=".", label='State 1', alpha=0.5)
    ax[1].errorbar(x, bern_2_h1, fmt='o', c='r', marker=".", label='State 2', alpha=0.5)
    ax[1].set_ylabel('Theta', rotation=90, fontsize=font2)
    ax[1].axhline(y=MASlocBern, linestyle=(0, (3, 5, 1, 5, 1, 5)), linewidth=1, color='b')
