...
class TestModel(unittest.TestCase):
    ...
    def estimationEvaluation(self):
        ...
        expon_state_censored_estimate = self.solver_expon_censored.estimate.E[0]

        self.assertTrue(abs(gamma_state_estimate.gamma_a - self.E_gamma.gamma_a) < 1)
        self.assertTrue(abs(gamma_state_estimate.gamma_scale - self.E_gamma.gamma_scale) < 1)
        
        self.assertTrue(abs(expon_state_estimate.exp_beta - self.E_expon.exp_beta) < 1)

        self.assertTrue(abs(gamma_state_censored_estimate.gamma_a - self.E_gamma.gamma_a) < 1)  
        self.assertTrue(abs(gamma_state_censored_estimate.gamma_scale - self.E_gamma.gamma_scale) < 1)
        
        self.assertTrue(abs(expon_state_censored_estimate.exp_beta - self.E_expon.exp_beta) < 1)
