...
class CellNode:
    ...
    def isLeaf(self):
        '''Returns True if the cell is a leaf of the tree with no daughter cell '''
    ...
    def isUnfinished(self):
        ...
        returns true when cell is still alive
        """
        return math.isnan(self.endT) and self.fate is None   
...
def generateLineageWithTime(initCells, experimentTime, locBern, cGom, scaleGom, 
    ...
    # initialize the list of the lineage with initial cells
        ...
        if cell.isUnfinished():
            ...
            if cell.endT < experimentTime: # determine fate only if endT is within range
                ...
                if switchT is not None and cell.startT > switchT: # when the cells should abide by the second set of parameters
                    ...
                    cell.fate = sp.bernoulli.rvs(locBern) 
                    
                # divide or die based on fate
                if cell.fate:
                    temp1, temp2 = cell.divide(cell.endT) 
                    # append the two new daughter cells to list
