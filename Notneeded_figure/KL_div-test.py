...
for num in populations: #a pop with num number of lineages
    ...
    for rep in range(reps):
        ...
        initCells = [num]
        X1 = gpt(experimentTime, initCells, locBern, cGom, scaleGom) #generate a pop with two cells
        X = remove_NaNs(X1)
        ...
        for lin in range(tHMMobj.numLineages): #go through each lineage for a thmm with a set of lineages
            ...
            EL_h
            for state in numstates:
                EL_h.append(EL_array[:,state])
            
            all cells after a start t are one state , use startT
